...
from .face_utils import warp_face_by_face_landmark_5, paste_back, FACE_MASK_BLUR, FACE_MASK_PADDING
...
def apply_enhance(crop_vision_frame, nets):
    ...
    if nets['is_onnx']:
        input_names = [input.name for input in frame_processor.get_inputs()]
        frame_processor_inputs = {}
        
        # Handle different input names for the main input
        if 'x' in input_names:
            frame_processor_inputs['x'] = crop_vision_frame
        elif 'input.1' in input_names:
            frame_processor_inputs['input.1'] = crop_vision_frame
        elif 'input' in input_names:
            frame_processor_inputs['input'] = crop_vision_frame
        
        # Handle weight input for models that require it
        if 'onnx::Cast_1' in input_names:
            frame_processor_inputs['onnx::Cast_1'] = np.array([1.0], dtype=np.float64)
        elif 'weight' in input_names:
            frame_processor_inputs['weight'] = np.array([1.0], dtype=np.float64)
